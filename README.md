...
На Windows — задача в Планировщике заданий: пошаговая инструкция в
[`docs/windows-worker.md`](docs/windows-worker.md) — ssh-ключ без пароля, установка
Hermes через `install.ps1`, обёртка `run_worker.cmd` с логом и автоперезапуском,
`schtasks` для старта при включении ПК (в том числе без входа в систему), чек-лист
приёмки и список Windows-граблей.

Отличия от Linux-варианта по существу: вместо systemd — Планировщик; у поллера
обязателен `--yolo` (иначе неинтерактивный запуск агента упрётся в подтверждение,
которого некому дать); ssh-ключ должен быть без пароля, потому что поллер ходит с
`BatchMode=yes`.
